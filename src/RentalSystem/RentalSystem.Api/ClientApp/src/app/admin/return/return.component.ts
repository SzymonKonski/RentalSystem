...
export class ReturnComponent implements OnInit {
  ...
  constructor(private http: HttpClient, private envUrl: EnvironmentUrlService, private route: ActivatedRoute, public adminService: AdminService) { }

  ngOnInit(): void {
    this.id = this.route.snapshot.params['rentId'];
    this.adminService.getPrice(this.id).subscribe(result => {
      this.price = result;
    },
  ...
  public sendFiles = (files, files2) => {
    ...
    this.http.post(this.envUrl.createCompleteRoute('api/reservations/admin/ReturnMe/' + this.id), formData, { reportProgress: true, observe: 'events' })
      .subscribe(event => {
